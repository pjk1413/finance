...
class retrieve_sentiment_data:
    ...
    def run_data_load(self, range='latest'):
        ...
        for i, entry in enumerate(self.list_of_stocks):
            sentiment_obj_list = self.parse_sentiment_obj(self.retrieve_data(entry[0], range))
            sql_statement_list = []
    ...
    def insert_sentiment_obj(self, sentiment_obj: sentiment_model):
        ...
        sql_statement = "INSERT INTO SENTIMENT_DATA (crawlDate, publishedDate, tickers, tags, source, url, title, " \
            "sent_neg, sent_pos, sent_neutral, sent_compound) " \
            f"VALUES ('{sentiment_obj.crawl_date}', '{sentiment_obj.published_date}', '{tickers}', '{tags}', " \
            f"'{sentiment_obj.source}', '{sentiment_obj.url}', '{title}', " \
    ...
    def insert_sentiment_reference(self, sentiment_obj: sentiment_model):
        select_url = "SELECT url FROM stock_sentiment_data.sentiment_data;"
        try:
            cursor = self.conn_sentiment.cursor()
            cursor.execute(select_url)
            cursor.close()
            self.conn_sentiment.close()
        except:
            print("ERROR")
