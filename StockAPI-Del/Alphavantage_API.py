...
class alphavantage_api:
    def __init__(self, symbol):
        self.sql = mysql_db()
        self.time_series = TimeSeries(key='KHWQTGUQ4KFEIFBM', output_format='pandas')
        self.market = None
    ...
    def test(self):
        data, meta_data = self.time_series.get_daily_adjusted(self.symbol, outputsize='compact')
        print(data)
    ...
    def get_valid_date_range(self, date_range):
    ...
    def get_stock_data(self, date_range):
        ...
        if date_range == 'full' or date_range == 'compact':
            try:
                data, meta_data = self.time_series.get_daily_adjusted(self.symbol, outputsize=self.date_range)
                self.validate_data(data)
                if (self.validate):
                    return self.clean_data(data)
                else:
                    print("Error with alpha-vantage data")
                    return -1
            except:
                self.sql.log_error("Error with alpha_vantage data")  # Needs more detail to be effective logging
                return -1
        else:
            mysql_db.log_error("Error: no valid date range given") # Needs more detail to be effective
            return -1