...
# Important Codes for Logging into Robin Hood
# ISADDZE3WCOKL2JG
# 281923 647235
class robinhood_api:
    ...
    def validate_data(self, data):
        ...
        for row in data:
            ...
            if row['open_price'] == 0 or row['close_price'] == 0 or row['high_price'] == 0 or row['low_price'] == 0 or row['volume'] == 0:
                self.validate = False
                return
    ...
    def clean_data(self, data):
        ...
        for row in data:
            ...
            low = row['low_price']
            volume = row['volume']
            date = datetime.datetime.strftime(datetime.datetime.strptime(row['begins_at'], "%Y-%m-%dT%H:%M:%SZ"), "%Y.%m.%d:%H.%M.%S")
    ...
    def get_stock_data(self, date_range):
        ...
        if date_range == '5year' or date_range == 'year' or date_range == '3month' or date_range == 'month' or date_range == 'week' or date_range == 'day':
            ...
            self.sql.log_error("Error: No valid date range given")
            return -1
